"use client";
import React from "react";
import { motion } from "framer-motion";
import { Sparkles, Eye, Palette, Award, Gem } from "lucide-react";

export default function DiamondGuideClient() {
  return (
    <div className="relative min-h-screen bg-[#0E0C0A] text-[#FAF7F2] pt-[180px] pb-24 px-6 md:px-12 overflow-hidden">
      
      {/* Ambient Glows */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#C9A84C]/4 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#C9A84C]/3 rounded-full blur-[120px] pointer-events-none" />
      
      <div className="max-w-6xl mx-auto space-y-24 relative z-10">
        
        {/* ── Header ── */}
        <motion.div 
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, ease: [0.4, 0, 0.2, 1] }}
          className="text-center space-y-6 max-w-4xl mx-auto"
        >
          <div className="badge-gold w-fit mx-auto">
            <Sparkles className="w-3.5 h-3.5 text-[#C9A84C]" />
            <span className="text-[10px] tracking-[0.2em] uppercase text-[#C9A84C] font-bold">The 4 C's</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-light text-[#FAF7F2] leading-tight">
            Diamond Colour & <span className="font-display italic gold-gradient-text">Clarity Guide</span>
          </h1>
          <p className="text-[#C4BAB0] font-light text-base md:text-lg leading-relaxed max-w-3xl mx-auto">
            The color and clarity of a diamond are two key factors to consider when choosing your perfect engagement ring.
          </p>
          <div className="gold-divider mx-auto" />
        </motion.div>

        {/* ── Content Blocks ── */}
        <div className="space-y-16">
          
          {/* Block 1: Clarity */}
          <motion.div 
            initial={{ opacity: 0, y: 32 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center"
          >
            <div className="order-2 md:order-1 glass-panel luxury-shadow p-10 rounded-3xl border border-[#C9A84C]/10 flex flex-col justify-center min-h-[300px] relative overflow-hidden">
               <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[#C9A84C]/5 to-transparent pointer-events-none" />
               <Eye className="w-10 h-10 text-[#C9A84C] mb-6" />
               <h3 className="text-2xl font-light text-[#FAF7F2] mb-4">Understanding Inclusions</h3>
               <p className="text-[#C4BAB0] text-sm leading-relaxed font-light">
                 Diamond clarity is the term used to measure naturally occurring 'inclusions' or tiny blemishes seen within a diamond at ten times magnification. This is measured on a diamond clarity scale which ranges from 'Flawless' to 'Included'.
               </p>
            </div>
            <div className="order-1 md:order-2 space-y-4">
               <span className="text-[10px] tracking-[0.32em] uppercase text-[#C9A84C] font-semibold">
                 01 / Clarity
               </span>
               <h2 className="text-3xl font-light text-[#FAF7F2] leading-tight">
                 A Window to <em className="italic text-[#C9A84C] not-italic" style={{ fontStyle: "italic" }}>Perfection</em>
               </h2>
               <p className="text-[#C4BAB0] text-sm leading-relaxed font-light">
                 At Vees Star, we examine every diamond under intense magnification to ensure the highest optical performance.
               </p>
            </div>
          </motion.div>

          {/* Block 2: Color */}
          <motion.div 
            initial={{ opacity: 0, y: 32 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center"
          >
            <div className="space-y-4">
               <span className="text-[10px] tracking-[0.32em] uppercase text-[#C9A84C] font-semibold">
                 02 / Color Grading
               </span>
               <h2 className="text-3xl font-light text-[#FAF7F2] leading-tight">
                 The D-Z <em className="italic text-[#C9A84C] not-italic" style={{ fontStyle: "italic" }}>Spectrum</em>
               </h2>
               <p className="text-[#C4BAB0] text-sm leading-relaxed font-light">
                 The natural color of a diamond is one of the standard 4Cs of diamond quality. White diamonds are available in colors ranging from D – Z, with D diamonds having no hue and displaying the greatest brilliance.
               </p>
            </div>
            <div className="glass-panel luxury-shadow p-10 rounded-3xl border border-[#C9A84C]/10 flex flex-col justify-center min-h-[300px] relative overflow-hidden">
               <div className="absolute top-0 left-0 w-32 h-32 bg-gradient-to-br from-[#C9A84C]/5 to-transparent pointer-events-none" />
               <Palette className="w-10 h-10 text-[#C9A84C] mb-6" />
               <h3 className="text-2xl font-light text-[#FAF7F2] mb-4">Vees Star Standards</h3>
               <p className="text-[#C4BAB0] text-sm leading-relaxed font-light mb-4">
                 This is measured using the Gemological Institute of America, or the GIA color scale. Within this range, our diamonds are only available between D and I colors.
               </p>
               <p className="text-[#C4BAB0] text-sm leading-relaxed font-light">
                 When you're looking for diamonds to purchase, the diamond color is one aspect of the buying process that you shouldn't overlook. Diamonds can vary hugely in price based on their color.
               </p>
            </div>
          </motion.div>

          {/* Block 3: Fancy Colors & GIA */}
          <motion.div 
            initial={{ opacity: 0, y: 32 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
            className="grid grid-cols-1 md:grid-cols-2 gap-8"
          >
            {/* Left Card: Fancy Colors */}
            <div className="glass-panel luxury-shadow p-10 rounded-3xl border border-[#C9A84C]/10 flex flex-col justify-center text-center relative overflow-hidden group">
               <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[#C9A84C]/5 to-transparent pointer-events-none group-hover:from-[#C9A84C]/10 transition-all duration-500" />
               <Gem className="w-10 h-10 text-[#C9A84C] mb-6 mx-auto" />
               <h3 className="text-2xl font-light text-[#FAF7F2] mb-4">Fancy-Colored Diamonds</h3>
               <p className="text-[#C4BAB0] text-sm leading-relaxed font-light">
                 Fancy-colored diamonds manifest in a range of tones including green, blue, yellow, orange, pink, and red. Red diamonds are the rarest, with only around 20 to 30 true red diamonds known to be in existence.
               </p>
            </div>

            {/* Right Card: GIA */}
            <div className="glass-panel luxury-shadow p-10 rounded-3xl border border-[#C9A84C]/10 flex flex-col justify-center text-center relative overflow-hidden group">
               <div className="absolute top-0 left-0 w-32 h-32 bg-gradient-to-br from-[#C9A84C]/5 to-transparent pointer-events-none group-hover:from-[#C9A84C]/10 transition-all duration-500" />
               <Award className="w-10 h-10 text-[#C9A84C] mb-6 mx-auto" />
               <h3 className="text-2xl font-light text-[#FAF7F2] mb-4">GIA Certification</h3>
               <p className="text-[#C4BAB0] text-sm leading-relaxed font-light">
                 The GIA is a renowned and well-respected diamond grading entity that issues reports for diamonds. It's advisable to seek a GIA certificate for every diamond you view. All solitaires purchased from Vees Star are accompanied by a GIA certificate.
               </p>
            </div>
          </motion.div>

        </div>
      </div>
    </div>
  );
}
